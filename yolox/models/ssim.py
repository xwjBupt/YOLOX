...
class SSIM_Loss(nn.Module):
    def __init__(self, in_channels, size=11, sigma=1.5):
    ...
    def forward(self, input, target, mask=None):
        ...
        mean2 = F.conv2d(
            target, self.weight, padding=self.size, groups=self.in_channels
        )
        ...
        sigma1_sq = (
            F.conv2d(
                input * input, self.weight, padding=self.size, groups=self.in_channels
            )
            - mean1_sq
        )
        sigma2_sq = (
            F.conv2d(
                target * target, self.weight, padding=self.size, groups=self.in_channels
            )
            - mean2_sq
        )
        sigma_12 = (
            F.conv2d(
                input * target, self.weight, padding=self.size, groups=self.in_channels
            )
            - mean_12
        )
        ...
        ssim = ((2 * mean_12 + C1) * (2 * sigma_12 + C2)) / (
